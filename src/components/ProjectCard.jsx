...
const ProjectsCard = ({ project, shown, last }) => {
    ...
    const handleClickPrimary = (e) => {
        e.preventDefault();
        if (project.primary.alert && project.primary.alert !== "none") {
            setAlertOpen(true);
        } else {
            ...
            <div className="row-1 col-1 font-arial flex flex-col-reverse group">
                <div className="bg-black text-white p-[clamp(0.75rem,_0.5739rem_+_0.7512vw,_1.25rem)] flex flex-col gap-6">
                    ...
                    <div className="flex justify-between items-end text-[clamp(1rem,_0.912rem_+_0.3756vw,_1.25rem)]">
                        <button className={`text-black bg-white px-[clamp(0.75rem,_0.662rem_+_0.3756vw,_1rem)] py-[clamp(0.5rem,_0.412rem_+_0.3756vw,_0.75rem)] duration-100 hover:bg-blue cursor-pointer ${project.primary.desktopOnly && window.innerWidth < 880 ? "opacity-50 pointer-events-none" : null}`} onClick={handleClickPrimary}>{project.primary.desktopOnly && window.innerWidth < 880 ? "Desktop Only" : project.primary.label}</button>
                            ...
                            <ul className="flex flex-row gap-[clamp(0.5rem,_0.412rem_+_0.3756vw,_0.75rem)]">
                                {
                                    project.colaborators.map((colaborator, index) => (
                                        <li key={index}><a className="bg-blue h-min px-1 py-0.5 text-white text-[clamp(0.875rem,_0.787rem_+_0.3756vw,_1.125rem)] duration-100 hover:bg-white hover:text-black" href={colaborator.link} target="_blank">{colaborator.name}</a></li>
                                ))
                            }
                            </ul>
                        </div>
